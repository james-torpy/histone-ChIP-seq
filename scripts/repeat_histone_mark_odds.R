...
descrip <- paste0("absolute_values_chapmanDE_all_ctls")
  ...
  res <- lapply(posits, function(Posit) {  
    ...
    if ( !file.exists(paste0(RobjectDir, 
      ...
      # format pc:
	    ...
	    pc_exp <- pc_exp[!(names(pc_exp) %in% c(pos_chapman_ctl, neg_chapman_ctl))]
    ...
    if ( 
     ...
     saveRDS(ctl_peak_data, file = paste0(RobjectDir,
      ...
      saveRDS(pc_peak_data, file = "/share/ScratchGeneral/jamtor/projects/hgsoc_repeats/histone-ChIP-seq/chapman-rothe_2013_hg19/Robjects/backup.rds")
