...
# load in repeat annotation:
#rp <- read.table(file = paste0(refDir, "/human-89.repeats.tab"), 
#	sep = "\t", fill = T, header = T)
...
# remove all entries with '_' or '-' in the name to get rid of alternate 
# chromosome entries and/or ERCC spike-ins:
#correct_chromosomes_underscore=grepl("_",rp$chr)
#correct_chromosomes_dash=grepl("-",rp$chr)
#correct_chromosomes_fullstop=grepl("[0-9].[0-9]",rp$chr)
#correct_chromosomes_MT=grepl("MT|M",rp$chr)
#both=correct_chromosomes_underscore|correct_chromosomes_dash|correct_chromosomes_fullstop|correct_chromosomes_MT
#correct_chromosomes=!both

## convert to GRanges object:
#rp_annot <- GRanges(
#  seqnames = rp$chr[correct_chromosomes],
#  ranges = IRanges(start=(rp$start[correct_chromosomes]), 
#    end=(rp$end[correct_chromosomes])),
#  strand = rp$strand[correct_chromosomes],
#  repeat_id = rp$type[correct_chromosomes]
#)
...
#start(ranges(test_annot))[start(ranges(test_annot)) >= 500
#  ] <- start(ranges(test_annot))[start(ranges(test_annot)) >= 500] - 500
#
## add 500 to the end of each ranges if the end is 500 bp or more from
## the end of the chromosome:
#end(ranges(test_annot))[end(ranges(test_annot)) <= 
#  (test_annot$seq_lengths - 500)] <- 
#    end(ranges(test_annot))[end(ranges(test_annot)) <= 
#      (test_annot$seq_lengths - 500)] + 500
...
######## below is too long #######
#min_annot <- rp_annot[rp_annot$repeat_id %in% names(peak_nos)]
#
#system.time(for ( i in 1:length(rp_annot) ) {
#    diff <- end(ranges(rp_annot))[i] - start(ranges(rp_annot))[i]
#    rand <- runif(1, 0, (rp_annot$seq_lengths[i] - diff) )
#    end(ranges(rp_annot))[i] <- rand + diff
#    start(ranges(rp_annot))[i] <- rand
#})
...
# make a data frame from repeat start and end vectors and split each row
# into list for efficient computation:
#ranges_df <- data.frame(start(rp_annot), end(rp_annot), 
#  seqnames(rp_annot), rp_annot$seq_lengths)

#system.time(
#  rp_list <- split(ranges_df, seq(nrow(ranges_df)))
#)

# make list of random ranges of same size as repeat ranges:
#### first check if each element of rp_list is data frame or vector! ####
#system.time(
#  lapply(rp_list, function(x)) {
#    diff <- x$end - x$start
#    rand <- runif(0, x$seqlengths)
#    return(c(rand, (rand + diff), seqnames(x), x$seqlengths))
#  }
#)
