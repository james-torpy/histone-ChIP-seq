...
for (o in 1:length(exp_nos)) {
  ...
  res <- lapply(posits, function(Posit) {  
    ...
    if ( !file.exists(paste0(RobjectDir, "/ctl_rp_", Posit, "_", Subset, "_", as.character(exp_nos[o]), "bp.rds")) ) {
      
      print("Creating expanded control repeat annotation...")
      # load repeats annotation:
      ctl_reps <- import(paste0(refDir, "custom3rep.hg19.gtf"))
...
saveRDS(res, paste0(RobjectDir, "/", exp_nos[o], "bp", descrip, "_peak_no_per_seq_plot_df.rds"))
    ...
    plot_df$group <- factor(plot_df$group, levels = c("chapman_upregulated_positive_control",
      "chapman_non-DE_negative_control", "chapman_downregulated_negative_control",
      "down_repeat_FDR<0.1", "down_repeat_FDR<0.05", "up_repeat_FDR<0.1", "up_repeat_FDR<0.05",
       "non_DE_repeat"))
