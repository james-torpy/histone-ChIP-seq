...
descrip <- paste0("absolute_values")
exp_nos <- c(500)
posits <- list("body")
...
pos_CPMR_ctl <- as.character(read.table(file=paste0(DE_dir, "/top_CPMR.txt"))[,1])
neg_CPMR_ctl <- as.character(read.table(file=paste0(DE_dir, "/bottom_CPMR.txt"))[,1])
...
for (o in 1:length(exp_nos)) {
  ...
  res <- lapply(posits, function(Posit) {  
    ...
    if ( !file.exists(paste0(RobjectDir, "/CPMR_ctls_", Posit, "_", 
      ...
      ctls <- gc[gc$gene_name %in% pos_CPMR_ctl|gc$gene_name %in% neg_CPMR_ctl|
        gc$gene_name %in% pos_chapman_ctl|gc$gene_name %in% neg_chapman_ctl|
...
saveRDS(res, paste0(RobjectDir, "/", exp_nos[o], "bp", descrip, "_peak_no_per_seq_plot_df.rds"))
}
...
#     plot_df$group[plot_df$id %in% pos_CPMR_ctl] <- "CPM_positive_control"
#     plot_df$group[plot_df$id %in% neg_CPMR_ctl] <- "CPM_negative_control"
#     plot_df$group[plot_df$id %in% pos_chapman_ctl] <- "chapman_positive_control"
#     plot_df$group[plot_df$id %in% neg_chapman_ctl] <- "chapman_negative_control"
