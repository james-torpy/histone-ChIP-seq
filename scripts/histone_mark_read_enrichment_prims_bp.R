### histone_mark_read_enrichment_prims_bp_enrichment.R ###
...
descrip <- paste0(sampleName, "_", exp_no, incl_marks, "_par")
...
if ( !file.exists(paste0(newRobjectDir, "/repeats_read_overlap_odds.rds")) ) {
  ...
  # (for length(rp_annot_test) = 19, 31.09 sec with parLapply, 
  # with lapply):
  system.time(
  ...
  saveRDS(rp_read_odds, file = paste0(newRobjectDir, "/repeats_read_overlap_odds.rds"))
  saveRDS(ctl_read_odds, file = paste0(newRobjectDir, "/ctl_read_overlap_odds.rds"))
  
} else {
  rp_read_odds <- readRDS(file = paste0(newRobjectDir, "/repeats_read_overlap_odds.rds"))
