...
inDir <- paste0(resultsDir, "/macs2/HGSOC_SAMN01761041/SRR600559-H3K27me3/")
...
inFile <- paste0(inDir, '“HGSOC_SAMN01761041”_peaks.broadPeak')

# specify if input file is bam or peak and process accordingly:
if ( bam_or_peak == "bam" ) {
  ...
  saveRDS(data_gr, paste0(RobjectDir, "/ChIP_gr.rds"))
...
if ( bam_or_peak == "bam") {
  ...
  rp_odds <- lapply(rp_annot, oddsRatio, data_gr)
  saveRDS(rp_odds, file = paste0(RobjectDir, "/rp_odds.rds"))
  
  ctl_odds <- lapply(ctls, oddsRatio, data_gr)
