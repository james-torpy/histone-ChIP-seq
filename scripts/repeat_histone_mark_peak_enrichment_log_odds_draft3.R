...
##########################################################################
### 3. Calculate odds ratios of peak enrichment in repeat and
# control regions ###
##########################################################################
...
if ( !file.exists(paste0(RobjectDir, "/repeats_peak_overlap_odds.rds")) ) {
  ...
  rp_peak_odds <- readRDS(file = paste0(RobjectDir, 
                                        "/repeats_peak_overlap_odds.rds"))
  ctl_peak_odds <- readRDS(file = paste0(RobjectDir, 
                                         "/ctl_peak_overlap_odds.rds"))
}

# # transform to log odds:
# rp_log_odds <- lapply(rp_peak_odds, function(x) {
#   x$odds <- log(x$odds)
#   colnames(x)[1] <- "log_odds"
#   return(x)
# })
# 
# ctl_log_odds <- lapply(rp_peak_odds, function(x) {
#   x$odds <- log(x$odds)
#   colnames(x)[1] <- "log_odds"
#   return(x)
# })
...
########################################################################
### 4. Plot log odds ratios of peak enrichment in repeat regions #
########################################################################

# create function to adjust y tick marks:
...
plot_log_odds <- function(odds, pval = p_thresh, sig_only = F, 
                          cont = NA, filter = T) {
  
  if ( any(odds$odds > 0 & odds$pval != 0) ) {
    
    if (filter) {
      
      # filter by odds:
      odds_DE <- odds[odds$odds != 0 & odds$pval != 0,]
      # filter by p-value:
      ...
      if ( is.list(cont) ) {
        ...
        # fetch bind positive and negative control data frames and 
        # annotate before binding with odds_sig:
      ...
      odds$ID <- rownames(odds)
      odds$thresh <- NA
      odds$thresh[odds$pval <= pval] <- "significant"
      odds$thresh[odds$pval > pval] <- "non-significant"
      odds$odds[odds$odds == 0] <- 1
      odds$std_error[odds$std_error == Inf] <- 0
        
      p <- ggplot(odds, aes(x = ID, y = odds))
      p <- p + geom_bar(aes(fill = thresh), stat = "identity")
      p <- p + geom_errorbar(aes(ymin=odds+std_error, 
                                   ymax=odds-std_error))
      p <- p + scale_y_continuous(trans = log_trans(), 
...
k=1
log_odds_p0.05 <- lapply(rp_peak_odds, plot_log_odds, pval = 0.05)

# plot DE odds with p < 0.1:
k=1
odds_p0.1 <- lapply(rp_peak_odds, plot_log_odds, pval = 0.1)

# plot controls with p < 0.05:
k=1
odds_ctls_p0.05 <- lapply(ctl_peak_odds, plot_log_odds, pval = 0.05)

# plot controls with p < 0.1:
k=1
odds_ctls_p0.1 <- lapply(ctl_peak_odds, plot_log_odds, pval = 0.1, filter = F)

# plot DE odds of repeats with DE RNA expression:
RNA_DE <- readRDS(file = paste0(DE_dir, "custom3_DEsigReps_names.rds"))
...
k=1
RNA_DE_odds_p0.05 <- lapply(RNA_DE_odds, plot_log_odds, pval = 0.05)

k=1
RNA_DE_odds_p0.05 <- lapply(RNA_DE_odds, plot_log_odds, pval = 0.05, cont = ctl_peak_odds)
...
l=1
lapply(odds_p0.05, function(x) {
  
  pdf(paste0(plotDir, "/", names(odds_p0.05)[l], "_repeat_peak_log_odds.pdf"))
  print(x[[2]])
  dev.off()
